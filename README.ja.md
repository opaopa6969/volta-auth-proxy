...
全ての[認証](docs/glossary/authentication-vs-authorization.ja.md)フローは**制約付き[ステートマシン](docs/glossary/state-machine.ja.md)**で駆動 — 不正な遷移が構造的に存在できない。エンジンは約120行、ビジネスロジックはゼロ。

> [ステートマシン](docs/glossary/state-machine.ja.md)の core は独立ライブラリ **[tramli](https://github.com/opaopa6969/tramli)** として公開済み。
> 設計パターンガイド: [STATE-MACHINE-PATTERN-GUIDE.md](docs/STATE-MACHINE-PATTERN-GUIDE.md)

```
flow/
  FlowEngine              ~120行、ビジネスロジックなし
  FlowDefinition           Builder DSL + build() 時 8項目検証

  oidc/     6 processors   OIDC ログインフロー (9状態)
  passkey/  4 processors   Passkey ログインフロー (6状態)
  mfa/      2 processors   MFA 検証フロー (4状態)
  invite/   4 processors   招待承認フロー (6状態)
```
