...
| **App 連携** | [ForwardAuth](docs/glossary/forwardauth.ja.md) + Internal API | 汎用 OIDC | SDK | 汎用 OIDC | Oathkeeper |
...
- **理解できる地獄を選ぶ** — Keycloak の設定地獄も、[OIDC](docs/glossary/oidc.ja.md) 自前実装の地獄も、どちらも地獄。ならスタックトレースが読める方を選ぶ。Auth だけは自分で持つ。何が起きているか分からないシステムにユーザーの認証を預けない
...
| [鍵ローテーション](docs/glossary/key-rotation.ja.md) | 管理 API で回転/失効。オーバーラップ期間付きの安全な移行 |
...
| 機能 | 詳細 |
|------|------|
| [テナント](docs/glossary/tenant.ja.md)解決 | 4 段階優先: [セッション](docs/glossary/session.ja.md) > サブドメイン > メールドメイン > 招待/手動 |
...
| 鍵の[暗号化](docs/glossary/encryption-at-rest.ja.md) | 秘密鍵は DB に AES-256-GCM で暗号化保存 |
| CSRF 保護 | HTML フォームはトークンベース。JSON API は [SameSite](docs/glossary/samesite.ja.md) + [Content-Type](docs/glossary/content-type.ja.md) で保護 |
...
| 機能 | 詳細 |
|------|------|
| [ForwardAuth](docs/glossary/forwardauth.ja.md) | App は HTTP ヘッダから identity を取得。認証コードゼロ |
...
Traefik の [ForwardAuth](docs/glossary/forwardauth.ja.md) ミドルウェアを設定（アーキテクチャセクション参照）。
...
| Phase | 状態 | 内容 |
|-------|------|------|
| **Phase 1: Core** | 開発中 | Google [OIDC](docs/glossary/oidc.ja.md), [テナント](docs/glossary/tenant.ja.md), ロール, 招待, [ForwardAuth](docs/glossary/forwardauth.ja.md), Internal API |
