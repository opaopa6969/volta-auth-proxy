...
[認証](docs/glossary/authentication-vs-authorization.ja.md)・テナント管理・[ロール](docs/glossary/role.ja.md)・[招待](docs/glossary/invitation-flow.ja.md)を一手に引き受け、[下流](docs/glossary/downstream-app.ja.md)の App は何もしなくてよくなります。

**[Keycloak](docs/glossary/keycloak.ja.md) 不要。oauth2-proxy 不要。制御こそ正義。**
...
[SaaS](docs/glossary/saas.ja.md) を作る。認証が必要。選択肢は:
...
| **App 連携** | [ForwardAuth](docs/glossary/forwardauth.ja.md) + Internal [API](docs/glossary/api.ja.md) | 汎用 OIDC | [SDK](docs/glossary/sdk.ja.md) | 汎用 OIDC | Oathkeeper |
| **設定の複雑さ** | .env + [YAML](docs/glossary/yaml.ja.md) 1 個 | 数百の設定項目 | ダッシュボード | 中程度 | 4 サービス |
| **外部依存** | [Postgres](docs/glossary/database.ja.md) のみ | Postgres + JVM | クラウド | Postgres/CRDB | Postgres + 複数 |
...
- **制御しやすいは正義** — 外部[サーバー](docs/glossary/server.ja.md)への依存を最小化
- **理解できる地獄を選ぶ** — [Keycloak](docs/glossary/keycloak.ja.md) の[設定地獄](docs/glossary/config-hell.ja.md)も、[OIDC](docs/glossary/oidc.ja.md) 自前実装の地獄も、どちらも地獄。ならスタックトレースが読める方を選ぶ。Auth だけは自分で持つ。何が起きているか分からないシステムにユーザーの[認証](docs/glossary/authentication-vs-authorization.ja.md)を預けない
- **密結合上等** — 1 プロセスで完結。マイクロサービス的な疎結合がもたらすのは「正しいアーキテクチャ」ではなく「設定と通信の複雑性」。認証系はレイテンシに敏感で障害が全体に波及する。ネットワークホップを減らし、デバッグを 1 箇所で完結させる
- **[ForwardAuth](docs/glossary/forwardauth.ja.md) パターン** — Proxy はリクエストボディを中継しない。認証チェックだけ
- **App のやることは 2 つだけ** — ヘッダを読む or [API](docs/glossary/api.ja.md) を叩く
...
| [セッション](docs/glossary/session.ja.md)管理 | 署名付き [Cookie](docs/glossary/cookie.ja.md)。8 時間スライディング。ユーザーあたり最大 5 同時セッション |
...
| サイレントリフレッシュ | volta-sdk-js が [HTTPステータスコード](docs/glossary/http-status-codes.ja.md) 401 で自動リフレッシュ。通常利用中にログイン画面を見ない |
...
| 機能 | 詳細 |
|------|------|
| [テナント](docs/glossary/tenant.ja.md)解決 | 4 段階優先: [セッション](docs/glossary/session.ja.md) > サブドメイン > メールドメイン > [招待](docs/glossary/invitation-flow.ja.md)/手動 |
| フリーメール除外 | gmail.com, outlook.com 等は自動でドメインマッチング対象外 |
| 複数テナント所属 | 1 ユーザーが複数テナントに異なる[ロール](docs/glossary/role.ja.md)で所属可能 |
| テナント切替 | セッション中に [API](docs/glossary/api.ja.md) で切替。ページリロードでクリーンな状態に |
...
| App 別制御 | volta-config.yaml で App ごとに許可ロールを定義。[ForwardAuth](docs/glossary/forwardauth.ja.md) で強制 |
| テナントスコープ | [ロール](docs/glossary/role.ja.md)はテナントごと。テナント A では ADMIN、テナント B では VIEWER が可能 |
...
| CSRF 保護 | [HTML](docs/glossary/html.ja.md) フォームはトークンベース。[JSON](docs/glossary/json.ja.md) API は [SameSite](docs/glossary/samesite.ja.md) + [Content-Type](docs/glossary/content-type.ja.md) で保護 |
| [レート制限](docs/glossary/rate-limiting.ja.md) | ログイン: 10/分/IP。API: 200/分/ユーザー |
...
| 監査ログ | 全認証イベント: ログイン、ログアウト、[ロール](docs/glossary/role.ja.md)変更、招待、セッション失効 |
...
| 機能 | 詳細 |
|------|------|
| [ForwardAuth](docs/glossary/forwardauth.ja.md) | App は [HTTP](docs/glossary/http.ja.md) ヘッダから identity を取得。認証コードゼロ |
| Internal [API](docs/glossary/api.ja.md) | ユーザー/テナント/メンバー CRUD の REST API |
| volta-sdk-js | ブラウザ [SDK](docs/glossary/sdk.ja.md) (~150 行)。401 自動リフレッシュ、テナント切替、ログアウト |
| volta-sdk ([Java](docs/glossary/java.ja.md)) | Javalin ミドルウェアで JWT 検証 |
...
| [招待](docs/glossary/invitation-flow.ja.md)管理 | 作成、一覧、取消。リンクコピー、QR コード |
...
| Phase | 状態 | 内容 |
|-------|------|------|
| **Phase 1: Core** | 開発中 | Google [OIDC](docs/glossary/oidc.ja.md), [テナント](docs/glossary/tenant.ja.md), [ロール](docs/glossary/role.ja.md), [招待](docs/glossary/invitation-flow.ja.md), [ForwardAuth](docs/glossary/forwardauth.ja.md), Internal [API](docs/glossary/api.ja.md) |
| Phase 2: Scale | 計画中 | 複数 [IdP](docs/glossary/idp.ja.md) (GitHub, Microsoft), M2M ([Client Credentials](docs/glossary/client-credentials.ja.md)), Redis [セッション](docs/glossary/session.ja.md), Webhook, **パスキー ([WebAuthn](docs/glossary/webauthn.ja.md)/FIDO2)** |
| Phase 3: Enterprise | 計画中 | [SAML](docs/glossary/sso.ja.md), メール通知, **[MFA](docs/glossary/mfa.ja.md)/2FA ([TOTP](docs/glossary/totp.ja.md), WebAuthn)**, i18n, 管理 UI 拡張, **条件付きアクセス (リスクベース認証)** |
...
| トレンド | Phase | 方針 |
|---------|-------|------|
| **パスキー ([WebAuthn](docs/glossary/webauthn.ja.md)/FIDO2)** | Phase 2 | パスワードレス認証の主流。Google ログインと並ぶ第 2 の認証手段として追加 |
| **[MFA](docs/glossary/mfa.ja.md)/2FA ([TOTP](docs/glossary/totp.ja.md))** | Phase 3 | Google Authenticator 等の TOTP。[テナント](docs/glossary/tenant.ja.md)管理者が「MFA 必須」を設定可能に |
| **リスクベース認証** | Phase 3 | 新しいデバイス/IP からのアクセス時に追加認証。`amr` claim で認証強度を [JWT](docs/glossary/jwt.ja.md) に反映 |
...
| **モバイル [SDK](docs/glossary/sdk.ja.md) (iOS/Android)** | Phase 4 | ネイティブ iOS/Android SDK。招待フローのディープリンク対応。生体認証連携 |
| **[SAML](docs/glossary/sso.ja.md) [SSO](docs/glossary/sso.ja.md)** | Phase 3 | エンタープライズ顧客の社内 [IdP](docs/glossary/idp.ja.md) (Active Directory 等) と連携 |
...
4. 該当なし → [招待](docs/glossary/invitation-flow.ja.md)コード入力 or テナント選択画面
...
| `DB_PORT` | 54329 | Postgres [ポート](docs/glossary/port.ja.md) |
...
| `GOOGLE_REDIRECT_URI` | http://localhost:7070/callback | [オープンリダイレクト](docs/glossary/open-redirect.ja.md)対策済み OAuth リダイレクト URI |
