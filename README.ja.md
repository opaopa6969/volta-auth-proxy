...
リクエストが来るたびに volta が認証チェックを行います。
...
[Terraform](docs/glossary/terraform.ja.md) が **Infrastructure as Code** をもたらした。volta は **Auth as Code** をもたらす。

[認証](docs/glossary/authentication-vs-authorization.ja.md)フローは [Java](docs/glossary/java.ja.md) コード — バージョン管理され、PR でレビューされ、CI でテストされ、パイプラインでデプロイされる。XML でもなく、管理画面でもなく、ビジュアルエディタでもない。**コード**。

| | [Keycloak](docs/glossary/keycloak.ja.md) | [Authentik](docs/glossary/authentik.ja.md) | [Auth0](docs/glossary/auth0.ja.md) | **volta** |
|---|---|---|---|---|
| フロー定義 | 管理 UI | ビジュアルエディタ | Rules/Actions | **コード** ([tramli](https://github.com/opaopa6969/tramli) 3.7.1) |
| 変更プロセス | UI でクリック | ドラッグ&ドロップ | ダッシュボード | **PR → レビュー → デプロイ** |
| 検証 | 実行時 | 実行時 | 実行時 | **定義時** ([`build()`](docs/glossary/state-machine.ja.md)) |
| ロールバック | 手動 | 手動 | 手動 | **`git revert`** |
| プラグイン | プラグイン SDK | 組み込み | Extensions | **[tramli-plugins](https://github.com/opaopa6969/tramli-plugins) 3.6.1**（監査、lint、オブザーバビリティ） |

tramli プラグインにより、フローコードに触れずに横断的関心事を追加できます:

- **監査プラグイン** — 全ての[状態](docs/glossary/state.ja.md)[遷移](docs/glossary/transition.ja.md)をアクター、タイムスタンプ、`durationMicros`（パフォーマンス計測用）付きで記録
- **lint プラグイン** — ビルド時にフロー定義を検証（到達不能状態、エラーハンドラの欠落等）
- **オブザーバビリティプラグイン** — `externallyProvided` アノテーションにより、外部データソースの接続を実行時ではなく定義時に検証

> volta は [Keycloak](docs/glossary/keycloak.ja.md) に対して [SQLite](docs/glossary/sqlite.ja.md) が [PostgreSQL](docs/glossary/postgresql.ja.md) に対するのと同じ — 軽量で組み込み型。フル機能の代替ではない。[SAML](docs/glossary/sso.ja.md)、[SCIM](docs/glossary/scim.ja.md)、ビジュアルフローエディタが必要なら [Keycloak](docs/glossary/keycloak.ja.md) か [Authentik](docs/glossary/authentik.ja.md) を。

***

## ステートマシン・アーキテクチャ

全ての[認証](docs/glossary/authentication-vs-authorization.ja.md)フローは **[tramli](https://github.com/opaopa6969/tramli) 3.7.1** — 制約付き[ステートマシン](docs/glossary/state-machine.ja.md)エンジンで駆動。不正な遷移が構造的に存在できない。以下の図は `MermaidGenerator.generate(definition)` で**コードから自動生成** — 古くなることがない。

全フローは統合された **AUTH-010 `AuthFlowHandler`** で処理される。以前の `OidcFlowRouter` + `MfaFlowRouter` の分離を統合し、ルーティングの簡素化とエラーハンドリングの一貫性を実現。

[tramli-plugins](https://github.com/opaopa6969/tramli-plugins) 3.6.1 により、監査・lint・オブザーバビリティを横断的プラグインとして全フローに適用。
...
新しい[デバイス](docs/glossary/device-fingerprint.ja.md)からの[ログイン](docs/glossary/login.ja.md)を[Persistent Cookie](docs/glossary/cookie.ja.md) (`__volta_device_trust`) で検知。[テナント](docs/glossary/tenant.ja.md)管理者が対応を設定:

| リスクレベル | 新デバイス | アクション |
|------------|-----------|----------|
| 1-3 | No | 何もしない — 通常[ログイン](docs/glossary/login.ja.md) |
| 1-3 | Yes | メール通知 |
| 4+ | Any | [MFA](docs/glossary/mfa.ja.md) 再検証（ステップアップ） |
| 5 | Any | ブロック |

リスクスコアリングは外部[不正検知](docs/glossary/fraud-detection.ja.md)サービスに委譲 (`FRAUD_ALERT_URL`)。[フェイルオープン](docs/glossary/fail-open.ja.md): サービスが落ちていたらリスク = 1（安全）。タイムアウト 3秒。

### [GDPR](docs/glossary/gdpr.ja.md) 対応

| 機能 | 詳細 |
|------|------|
| データエクスポート | `POST /api/v1/users/me/data-export` → [JSON](docs/glossary/json.ja.md) ダウンロード |
| 削除権 | `DELETE /api/v1/users/me` → 30日猶予後に完全削除 |
| [監査ログ](docs/glossary/audit-log.ja.md)匿名化 | 削除時: actor_id → NULL、PII を detail から除去 |

### [i18n](docs/glossary/i18n.ja.md)（国際化）

全ての UI 文字列を `messages_ja.properties` / `messages_en.properties` に外部化。ユーザー設定 > `Accept-Language` [ヘッダ](docs/glossary/header.ja.md) > デフォルト (ja) の優先順位で[ロケール](docs/glossary/locale.ja.md)解決。

### [ポリシーエンジン](docs/glossary/policy-engine.ja.md)

[ロール](docs/glossary/role.ja.md)[階層](docs/glossary/hierarchy.ja.md)と[権限](docs/glossary/permission.ja.md)は `dsl/policy.yaml` から読み込み。Main.java のハードコード[ロール](docs/glossary/role.ja.md)チェック（約30箇所）を `policy.enforceMinRole()` / `policy.enforce()` に置換済み。
...
| [デバイストラスト](docs/glossary/device-fingerprint.ja.md) | [Persistent Cookie](docs/glossary/cookie.ja.md) で既知デバイスを識別。新デバイス → 通知 or [ステップアップ](docs/glossary/step-up-authentication.ja.md) |
| [不正検知](docs/glossary/fraud-detection.ja.md) | 外部 API によるプラガブルなリスクスコアリング。[フェイルオープン](docs/glossary/fail-open.ja.md)、3秒タイムアウト |
| [PII](docs/glossary/pii.ja.md) マスク | `@Sensitive` フィールドが[監査](docs/glossary/audit-log.ja.md)遷移ログで自動マスク |
| [GDPR](docs/glossary/gdpr.ja.md) | データエクスポート + 削除権（30日猶予）。[監査](docs/glossary/audit-log.ja.md)匿名化 |
...
| [Redis](docs/glossary/redis.md) | 7+ | [セッション](docs/glossary/session.ja.md)ストア + tramli-viz テレメトリ |
| [Docker](docs/glossary/docker.ja.md) | 24+ | [Postgres](docs/glossary/database.ja.md) / [Redis](docs/glossary/redis.md) 用（ローカルインストールがあれば不要） |

### 技術スタック

| コンポーネント | 選択 | 理由 |
|---------------|------|------|
| 言語 | [Java](docs/glossary/java.ja.md) 21 | [LTS](docs/glossary/lts.md)、成熟したエコシステム |
| [ビルド](docs/glossary/build.ja.md) | **[Maven](docs/glossary/maven.ja.md)** | [Java](docs/glossary/java.ja.md) バージョンアップ時の安定性（Gradle ではなく） |
| Web | [Javalin](docs/glossary/javalin.md) 6.7 | 軽量、~200ms 起動 |
| [ステートマシン](docs/glossary/state-machine.ja.md) | [tramli](https://github.com/opaopa6969/tramli) 3.7.1 + [tramli-plugins](https://github.com/opaopa6969/tramli-plugins) 3.6.1 | 制約付きステートマシンエンジン + 監査/lint/オブザーバビリティプラグイン |
| DB | [Postgres](docs/glossary/database.ja.md) 16 | 信頼性、[監査ログ](docs/glossary/audit-log.ja.md)に JSONB |
| キャッシュ / テレメトリ | [Redis](docs/glossary/redis.md) 7 | [セッション](docs/glossary/session.ja.md)ストア、tramli-viz 用 [Pub/Sub](docs/glossary/pubsub.md) テレメトリ |
...
volta-auth-proxy は[マルチテナント](docs/glossary/multi-tenant.ja.md)を後付けではなく、設計の中核として扱います。
...
- `memberships` テーブルがユーザーと[テナント](docs/glossary/tenant.ja.md)を結合
- [ロール](docs/glossary/role.ja.md)は**[テナント](docs/glossary/tenant.ja.md)ごと**: テナント A では ADMIN、テナント B では VIEWER ということが可能

### ログイン後のテナント選択

[OIDC](docs/glossary/oidc.ja.md) [認証](docs/glossary/authentication-vs-authorization.ja.md)完了後:

1. 所属[テナント](docs/glossary/tenant.ja.md)が **1つ** の場合 — 自動選択、追加ステップなし
2. 所属[テナント](docs/glossary/tenant.ja.md)が **複数** の場合 — [テナント](docs/glossary/tenant.ja.md)選択画面を表示 ([📊](dge/specs/ui-flow.md#flow-3-tenant-selection))
3. 所属[テナント](docs/glossary/tenant.ja.md)が **ない** 場合 — [招待](docs/glossary/invitation-flow.ja.md)コード入力または手動選択

### テナント分離

全ての [API](docs/glossary/api.ja.md) リクエストは `X-Volta-Tenant-Id` [ヘッダ](docs/glossary/header.ja.md)でアクティブ[テナント](docs/glossary/tenant.ja.md)にスコープされます:

- [ForwardAuth](docs/glossary/forwardauth.ja.md) が全リクエストに `X-Volta-Tenant-Id` を注入
- [テナント](docs/glossary/tenant.ja.md)スコープの [API](docs/glossary/api.ja.md) は `/api/v1/tenants/{tenantId}/*` パスを使用 — パスの `tenantId` と [JWT](docs/glossary/jwt.ja.md) の `volta_tid` [claim](docs/glossary/claim.md) が一致する必要あり
- [クロステナントアクセス](docs/glossary/cross-tenant-access.ja.md)は**構造的に防止**（ポリシーだけでなく）

### 招待フロー

新メンバーは[招待](docs/glossary/invitation-flow.ja.md)を通じて[テナント](docs/glossary/tenant.ja.md)に参加:

1. ADMIN 以上が[招待](docs/glossary/invitation-flow.ja.md)を作成（リンクまたは QR コード）
2. 招待者がリンクをクリックし、[同意画面](docs/glossary/consent-screen.ja.md)を確認 ([📊](dge/specs/ui-flow.md#flow-1-invite-link---first-login))
3. 承認すると、指定された[ロール](docs/glossary/role.ja.md)で `membership` が作成される
4. アカウントがない場合は、先に [OIDC](docs/glossary/oidc.ja.md) [ログイン](docs/glossary/login.ja.md)が実行される

***

## volta-gateway 互換性

[volta-gateway](https://github.com/opaopa6969/volta-gateway) は Rust 製の[リバースプロキシ](docs/glossary/reverse-proxy.ja.md)で、volta-auth-proxy 互換の認証サーバーを内蔵しています。

| | Traefik + volta-auth-proxy | volta-gateway |
|---|---|---|
| プロトコル | [ForwardAuth](docs/glossary/forwardauth.ja.md) | 組み込み（同じ[ヘッダ](docs/glossary/header.ja.md)） |
| [ヘッダ](docs/glossary/header.ja.md) | `X-Volta-*` | `X-Volta-*`（同一） |
| デプロイ | 2 サービス | 1 サービス |
| ユースケース | 既存の Traefik 環境 | 新規デプロイ、シンプルさ重視 |

volta-gateway は同じ [ForwardAuth](docs/glossary/forwardauth.ja.md) プロトコルを使い、同じ `X-Volta-*` [ヘッダ](docs/glossary/header.ja.md)を生成します。[下流](docs/glossary/downstream-app.ja.md)の[アプリ](docs/glossary/downstream-app.ja.md)はどちらの構成でも同じように動作します — コード変更は不要です。

**ドロップイン置換:** 現在 Traefik + volta-auth-proxy を使っている場合、DNS を gateway に向けるだけで volta-gateway に移行できます。[アプリ](docs/glossary/downstream-app.ja.md)は同じ[ヘッダ](docs/glossary/header.ja.md)を読み続けます。

***

## セキュリティ監査

21 件の[セキュリティ](docs/glossary/security-responsibility.ja.md)課題を監査・修正済み。[OWASP](docs/glossary/owasp.md) ガイドラインに準拠:

| カテゴリ | 修正内容 |
|---------|---------|
| [PKCE](docs/glossary/pkce.ja.md) | コード検証子を保存時に[暗号化](docs/glossary/encryption.ja.md)（ハッシュだけでなく） |
| タイミング攻撃 | 全[トークン](docs/glossary/token.ja.md)比較に定数時間比較を使用（`MessageDigest.isEqual`） |
| [XXE](docs/glossary/xxe.md) | 不要な XML パースを無効化、外部エンティティ解決をブロック |
| [GDPR](docs/glossary/gdpr.ja.md) | 30 日猶予後のハードデリート。ユーザー削除時の[監査ログ](docs/glossary/audit-log.ja.md)匿名化 |
| [レート制限](docs/glossary/rate-limiting.ja.md) | 全[エンドポイント](docs/glossary/endpoint.ja.md)に適用 — [ログイン](docs/glossary/login.ja.md)（10/分/IP）、[API](docs/glossary/api.ja.md)（200/分/ユーザー）、[招待](docs/glossary/invitation-flow.ja.md)承認 |
| [セッション](docs/glossary/session.ja.md) | [固定攻撃](docs/glossary/session-fixation.ja.md)防止、同時[セッション](docs/glossary/session.ja.md)制限（5）、[スライディング](docs/glossary/sliding-window-expiry.ja.md)有効期限 |
...
全ての[状態](docs/glossary/state.ja.md)[遷移](docs/glossary/transition.ja.md)が `durationMicros` 付きの構造化ログを生成。パフォーマンス監視に活用:
...
tramli-viz が [Redis](docs/glossary/redis.md) [Pub/Sub](docs/glossary/pubsub.md) 経由でリアルタイムのフロー[遷移](docs/glossary/transition.ja.md)イベントを受信:

- **`/viz/flows`** — ライブフロー可視化ダッシュボード
- **`/api/v1/admin/flows/{flowId}/transitions`** — 特定フロー実行の遷移をリプレイ（デバッグ用）

### 管理者向けフローエンドポイント

| メソッド | パス | 説明 |
|---------|------|------|
| GET | `/viz/flows` | フロー可視化 UI（tramli-viz） |
| GET | `/api/v1/admin/flows` | 最近のフロー実行一覧 |
| GET | `/api/v1/admin/flows/{flowId}/transitions` | 特定フローの遷移リプレイ |
