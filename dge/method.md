...
DGE と素の LLM レビューは補完関係にある。どちらか一方では片手落ち。

```
DGE の得意領域（判断）:
  - アーキテクチャ判断（作る/作らない、構成の簡素化）
  - 前提を問う（「なぜ JWT？」「Keycloak 要る？」）
  - 責務分担の仕分け（DELEGATE / HYBRID / BUILD）
  - Phase 構成の設計（何を先に、何を後に）

素の LLM の得意領域（詳細）:
  - セキュリティの見落とし（CSRF、XSS、権限昇格）
  - 具体値の決定（Rate Limit 値、トークン有効期限）
  - DB スキーマの完全な SQL 定義
  - コードレベルの実装例
  - 環境変数一覧の網羅
