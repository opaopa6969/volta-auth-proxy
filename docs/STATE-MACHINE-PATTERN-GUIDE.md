...
```mermaid
flowchart TB
    HTTP["HTTP / Event Layer<br/>(Router が Flow を起動する)"]
    Engine["FlowEngine (汎用、ビジネスロジックなし)<br/>- auto chain (Auto/Branch 連鎖実行)<br/>- guard validation (External 遷移)<br/>- transition logging (監査)"]
    Def["FlowDefinition (宣言的遷移テーブル)<br/>「何が→何に→どう遷移するか」の全体地図"]
    Proc["Processor / Guard / BranchProcessor<br/>(1遷移 = 1クラス、ビジネスロジックの居場所)"]
    Ctx["FlowContext (Accumulator)<br/>型安全なデータバケツ、requires/produces 契約"]
    StateE["FlowState enum (状態定義)<br/>コンパイラが網羅性を保証"]

    HTTP --> Engine --> Def --> Proc --> Ctx --> StateE
```
