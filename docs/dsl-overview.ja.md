...
> volta DSL は認証動作の **唯一の真実の源 (single source of truth)**。
> 実装はドライバー。DSL が仕様。
...
DSL は [DGE](../dge/sessions/) セッション（106 Gap 発見・解決）で設計され、3 回の tribunal review を経て最終スコア 10/10 を獲得しました。

### DSL と実装の関係

DSL は Java コードを置き換えるものではありません。並走します:
...
1. **Java コードは動いている。** Phase 1-4 実装済み、テスト通過。壊す理由がない
2. **DSL ランタイムがまだ存在しない。** 作ること自体が 1 プロジェクト
3. **DSL の目的は仕様であり実行ではない。** tribunal review で確定: 3 つの目的は AI 仕様伝達・テスト生成・ドキュメント生成。ランタイム実行ではない
4. **「制御しやすいは正義」** DSL ランタイム層を挟むとデバッグが 2 箇所に。今は Java だけ見ればいい
...
| ファイル | 目的 |
|---------|------|
| [`dsl/auth-machine-phase2-4.yaml`](../dsl/auth-machine-phase2-4.yaml) | 追加状態: [MFA](glossary/mfa.ja.md), [SAML](glossary/sso.ja.md), M2M, Webhook |
| [`dsl/volta-config.schema.yaml`](../dsl/volta-config.schema.yaml) | volta-config.yaml の JSON Schema バリデーション |
...
volta DSL は常に**マスター**。評価エンジンは**ドライバー** — [Interface](glossary/interface-extension-point.ja.md) で差し替え可能:
...
| Phase | ドライバー | 種別 | 依存 |
|-------|-----------|------|------|
| **Phase 1-3** | `JavaPolicyEvaluator` | Pure Java 直接評価 | なし |
| **Phase 4 案 A** | `CasbinPolicyEvaluator` | [jCasbin](https://github.com/casbin/jcasbin) | Pure Java, Maven Central |
...
**Phase 4 推奨: jCasbin** — Pure Java、追加インフラなし、「密結合上等」哲学に合致。
...
1. **構造** — ファイルごとのスキーマ検証
2. **ファイル間参照** — errors.yaml が唯一の定義場所
...
全状態遷移は [ui-flow.md](../dge/specs/ui-flow.md) で mermaid により可視化:
