...
プロダクトとしてではなく、**学習プラットフォーム**として。585 ファイル（EN 293 / JA 292）の[用語集](../docs/glossary/)（専門家 → おばあちゃんレベル）。認証の全体像を可視化した [DSL](dsl-overview.ja.md)。「分かったフリが一番危険」哲学。
...
自前が怖い          → volta（DSL + 585 ファイル + DGE 設計）
