...
```
ブラウザ → Traefik → ForwardAuth GET /auth/verify → volta-auth-proxy
                                                       ├─ 200 + X-Volta-* headers
                                                       └─ 302 /login?return_to=...
```
...
└───────────────┬────────────────────────────────────────────────────┘
                │ HTTPS
┌───────────────▼────────────────────────────────────────────────────┐
│ Traefik v3.3 / volta-gateway (Rust reverse proxy)                  │
│   - Routes                                                          │
│   - ForwardAuth middleware ──────────────────────────┐              │
└─────────────────────────────────────────────────────┼──────────────┘
                                                      │ GET /auth/verify
┌─────────────────────────────────────────────────────▼──────────────┐
│ volta-auth-proxy (Javalin + tramli)  :7070                          │
│                                                                     │
│  before-filters:                                                    │
│    CORS → i18n (Accept-Language) → RequestId →                     │
│    RateLimiter → CSRF check → MFA pending redirect →               │
│    TenantMFA enforcement                                            │
│                                                                     │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │ AuthFlowHandler (/auth/verify /login /callback /mfa/*)       │  │
│  └───────────────────────────┬──────────────────────────────────┘  │
│  ┌───────────────────────────▼──────────────────────────────────┐  │
│  │ AuthRouter (/auth/saml/* /auth/magic-link/* /auth/logout /   │  │
│  │            /auth/switch-tenant /settings/*)                  │  │
│  └───────────────────────────┬──────────────────────────────────┘  │
│  ┌───────────────────────────▼──────────────────────────────────┐  │
│  │ ApiRouter (/api/v1/*)  AdminRouter (/admin/*)                │  │
│  │ ScimRouter (/scim/v2/*) VizRouter (/viz/*)                   │  │
│  └───────────────────────────┬──────────────────────────────────┘  │
│                              │                                      │
│  ┌───────────────────────────▼──────────────────────────────────┐  │
│  │ Services                                                      │  │
│  │  AuthService · OidcService · SamlService · MfaService        │  │
│  │  SessionStore · JwtService · PolicyEngine · TenancyPolicy    │  │
│  │  LocalNetworkBypass · FraudAlertClient · DeviceTrustService  │  │
│  └───────────────────────────┬──────────────────────────────────┘  │
│                              │                                      │
│  ┌───────────────────────────▼──────────────────────────────────┐  │
│  │ tramli FlowEngine                                             │  │
│  │  OidcFlowDef · MfaFlowDef · PasskeyFlowDef · InviteFlowDef  │  │
│  │  Plugins: AuditStore · PolicyLint · Observability            │  │
│  └───────────────────────────┬──────────────────────────────────┘  │
│                              │                                      │
│  ┌───────────────────────────▼──────────────────────────────────┐  │
│  │ SqlStore (PostgreSQL via HikariCP + Flyway V1-V23)           │  │
│  │ SessionStore (postgres or redis)                             │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                                                                     │
│  OutboxWorker ──→ NotificationService (SMTP / SendGrid)            │
│  AuditService ──→ AuditSink (postgres / kafka / elasticsearch)     │
│  VizRouter ────→ Redis pub/sub (tramli-viz telemetry)              │
└─────────────────────────────────────────────────────────────────────┘
```
...
```
GET /auth/saml/login?idp_id={id}&return_to={url}
  → AuthnRequest(RequestId) + HMAC RelayState 生成
  → 302 IdP SSO URL

POST /auth/saml/callback
  → SamlService.parseIdentity(SAMLResponse, RelayState)
    ├─ XXE ハードニング (DTD 禁止)
    ├─ XSW 対策 (secureValidation=true)
    ├─ Issuer / Audience / NotOnOrAfter / RequestId / ACS URL 検証
    └─ RelayState HMAC 検証
  → 認証成功 → セッション発行
```
...
```
┌──────────────────────────────────────────────────────────────┐
│ 上位層: Session SM (永続 — sessions.auth_state)               │
│   AUTHENTICATING / AUTHENTICATED_MFA_PENDING / FULLY_AUTHENTICATED │
│   EXPIRED / REVOKED (terminal)                               │
└────────────────────────┬─────────────────────────────────────┘
                         │ 起動・完了通知
┌────────────────────────▼─────────────────────────────────────┐
│ 下位層: Flow SMs (一時永続 — auth_flows テーブル)              │
│   OIDC · SAML · MFA · Passkey · Invite                       │
│   TTL: 5 min (OIDC/MFA/Passkey), 7 days (Invite)             │
...
```
HTTP リクエスト → AuthFlowHandler
  ├─ セッションなし → OidcFlowDef.start() → auth_state = AUTHENTICATING
  ├─ auth_state = AUTHENTICATING → OIDC/Passkey フロー再開
  ├─ auth_state = AUTHENTICATED_MFA_PENDING → MfaFlowDef.start()
  └─ auth_state = FULLY_AUTHENTICATED → 200 OK (ForwardAuth パス)

Flow 完了イベント:
  OIDC COMPLETE        → Session SM: AUTHENTICATING → FULLY_AUTHENTICATED
  OIDC COMPLETE_MFA_PENDING → AUTHENTICATING → AUTHENTICATED_MFA_PENDING
  MFA VERIFIED         → AUTHENTICATED_MFA_PENDING → FULLY_AUTHENTICATED
  Passkey COMPLETE     → AUTHENTICATING → FULLY_AUTHENTICATED
  Invite COMPLETE      → メンバーシップ作成 (Session SM は別途 OIDC が管理)
```
...
```
GET /auth/saml/login?idp_id=...&return_to=...
  └─ AuthnRequest 生成 + RequestId 保存
  └─ HMAC-signed RelayState (JSON: tenant_id, return_to, nonce)
  └─ 302 IdP SSO URL

POST /auth/saml/callback (SAMLResponse, RelayState)
  └─ SamlService.parseIdentity()
  └─ Session 発行 or MFA へ
```
...
GET /auth/verify
  │
  ├─ [MFA pending check]
  │   session あり + mfaVerifiedAt null → 302 /mfa/challenge?return_to=...
  │
  ├─ [Session authenticate]
  │   Session cookie あり
  │   ├─ App policy チェック (X-Volta-App-Id / X-Forwarded-Host)
  │   │   アクセス不可 → 401
  │   ├─ URL-driven tenant re-scope
  │   │   (slug / subdomain / domain routing — TenancyPolicy 参照)
  │   ├─ JWT 発行 + X-Volta-* ヘッダー付与
  │   └─ 200 OK
  │
  ├─ [LocalNetworkBypass]
  │   Session なし + clientIp ∈ LOCAL_BYPASS_CIDRS
  │   → 200 OK + X-Volta-Auth-Source: local-bypass
  │
  └─ [No session, external IP]
      → 302 /login?return_to={X-Forwarded-Proto}://{X-Forwarded-Host}{X-Forwarded-Uri}
```
...
volta-auth-proxy
  │
  ├─ ObservabilityPlugin → RedisTelemetrySink
  │                           → Redis pub/sub (volta:viz:events)
  │
  └─ VizRouter
       ├─ GET /viz/flows     — FlowDefinition JSON グラフ (全4フロー)
       ├─ GET /viz/replay/{id} — 過去フロー遷移リプレイ
       ├─ GET /viz/ws        — WebSocket ブリッジ (Redis → WS)
       └─ GET /viz/auth-events — SSE (volta:auth:events)
                                 → LOGIN_SUCCESS / LOGOUT を配信
```
