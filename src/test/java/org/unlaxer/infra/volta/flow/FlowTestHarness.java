...
public final class FlowTestHarness<S extends Enum<S> & FlowState> {
    ...
    public FlowTestHarness<S> forceState(S state) {
        ...
        flow = FlowInstance.restore(flow.id(), flow.sessionId(), definition,
                flow.context(), state, flow.createdAt(), flow.expiresAt(),
                flow.guardFailureCount(), flow.version(), flow.exitState());
        store.save(flow);
        return this;
    }
